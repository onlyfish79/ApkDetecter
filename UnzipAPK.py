...
class UnzipAPK():
    ...
    def __init__(self, apkPath):
        classesPath = apkPath+".FILES"
        cmd = 'unzip -q "%s" -d "%s"' % (apkPath, classesPath)
        if os.path.isdir(classesPath):
            print 'unzip file has exists, delete it'
            shutil.rmtree(classesPath)
        os.system(cmd)
        self.unpackDir = classesPath
        self.getdexcontent()
        #self.Init_dexHeader()
        #self.unpackxml()
        self.dexdump()
...
#    apkPath = r"/home/helen/APK/ApiTest-debug.baidu.encrypted.Signed.apk"
